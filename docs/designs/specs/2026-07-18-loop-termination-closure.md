...
| 26 | D4 note's residual-coverage claim was refuted by the critic — item 2 cannot catch the read-path-only half-update the note credited it with (the finding itself was supported) (P2) | Note rewritten: that residual is named and accepted, blast radius stated, not claimed caught |
