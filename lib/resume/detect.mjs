...
import {
  closeSync, openSync, readSync, readdirSync, statSync,
} from 'node:fs';
...
// is cosmetic; selection is unaffected.
export const LEDGER_READ_BYTES = 16 * 1024;

// Read at most this many ledgers per session start. Completed run directories
// accumulate, and reading every one keeps total cost O(runs). Above this bound
// only the most-recently-written ledgers are read — a resumable (recently active)
// run has a recent ledger mtime, while old completed runs sort last — so the
// hot-path read work is bounded regardless of history size. Pruning old runs is
// afk-lifecycle scope; this hook only bounds its own read cost.
export const MAX_LEDGER_SCAN = 512;
...
export function collectResumable(runsDir, {
  root, now, staleMinutes = STALE_MINUTES, maxScan = MAX_LEDGER_SCAN,
}) {
  let entries;
  try {
  ...
  if (dirs.length > maxScan) {
    // Too many run directories to read them all on the session-start hot path.
    // Read only the most-recently-written ledgers (statSync is far cheaper than
    // the read+parse it bounds); a resumable run's heartbeat writes keep its
    // ledger mtime recent, so it sorts ahead of old completed runs.
    const mtimeOf = (name) => {
      try {
        return statSync(join(runsDir, name, 'ledger.md')).mtimeMs;
      } catch {
        return 0; // no ledger / unreadable → sorts last, gets dropped first
      }
    };
    dirs = dirs
      .map((entry) => ({ entry, mtime: mtimeOf(entry.name) }))
      .sort((a, b) => b.mtime - a.mtime)
      .slice(0, maxScan)
      .map((x) => x.entry);
    // Never a silent cap: say what was skipped. stderr is a diagnostic, not the
    // injected context, and does not affect the hook's stdout contract.
    process.stderr.write(
      `[afk-resume-detect] ${entries.length} run directories; read the newest ${maxScan} `
      + 'ledgers and skipped older ones. Prune .afk/runs to keep session start fast.\n',
    );
  }
  ...
  for (const entry of dirs) {
    ...
    let parsed;
    try {
      parsed = parseLedger(readPrefix(ledgerPath, LEDGER_READ_BYTES));
    } catch {
...
export function buildContext(runs, { mode }) {
  ...
  if (runs.length === 1) {
    ...
    if (mode === 'auto') {
      lines.push(
        ...
        + 'do NOT drive if the run is now `complete`, if its heartbeat is fresh (a live tick '
        + 'already owns it), or if another live run holds this scope — surface it instead. '
        + 'Only when the run is still `state: active` with a stale heartbeat, refresh its UTC '
        + "heartbeat and run the full waterfall to the queue's end or the next auto-pause, "
  ...
  for (const run of runs) {
    lines.push(`  - **${run.runId}** — \`${run.relPath}\` (stale ${staleLabel(run)})`);
