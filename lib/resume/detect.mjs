  ...
  if (runs.length === 1) {
    ...
    if (mode === 'auto') {
      lines.push(
        ...
        + 'elsewhere, resume this run autonomously per the afk skill. FIRST re-read this '
        + "run's ledger and confirm it is still `state: active` with a heartbeat still stale "
        + 'beyond the ~20-min overlap guard: another session may have claimed it between '
        + 'session start and now, so if the heartbeat is now fresh (a live tick owns it) or '
        + 'the state is `complete`, do NOT drive it — just note it is being handled. If you '
        + 'cannot confirm you are the only session resuming it (another window may have '
        + 'started at the same moment), also do NOT auto-drive — surface it for the operator '
        + 'instead. Otherwise refresh its UTC heartbeat, then run the full waterfall to the '
        + "queue's end or the next auto-pause, honoring the run's merge policy. If the "
        + 'operator asks for something else, do that and note this run is resumable.',
      );
    } else {
      lines.push(
