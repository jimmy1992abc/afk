...
// state, heartbeat, and a `scope:` line) lives there, so bounding the read keeps
// SessionStart cost flat as completed-run directories accumulate — instead of
// reading every entire ledger on every session start. A `## scope` block past
// this bound is missed, but scope is cosmetic; selection is unaffected.
