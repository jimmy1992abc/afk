...
// Whole minutes between `now` (a Date) and an ISO-8601 heartbeat, or null when
// the heartbeat is missing or unparseable — which the selector treats as stale.
...
export function collectResumable(runsDir, { root, now, staleMinutes = STALE_MINUTES }) {
  ...
  for (const entry of entries) {
    ...
    const stale = staleMinutesOf(parsed.heartbeat, now);
    if (stale !== null && stale < staleMinutes) continue; // fresh → a live tick owns it
    selected.push({
