...
test('staleMinutesOf computes whole minutes from an ISO-8601 UTC heartbeat', () => {
  assert.equal(staleMinutesOf(iso(30), NOW), 30);
  assert.equal(staleMinutesOf(iso(0), NOW), 0);
});

test('staleMinutesOf returns null for a missing or garbled heartbeat', () => {
  assert.equal(staleMinutesOf('', NOW), null);
  assert.equal(staleMinutesOf('not-a-date', NOW), null);
  assert.equal(staleMinutesOf(undefined, NOW), null);
});
