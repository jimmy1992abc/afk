...
import {
  mkdtempSync, mkdirSync, rmSync, utimesSync, writeFileSync,
} from 'node:fs';
...
test('collectResumable caps how many ledgers it reads, newest ledger mtime first', () => {
  withRoot((root) => {
    // three stale runs with distinct ledger mtimes; the oldest must be dropped
    // when the scan is capped below the run count, keeping the hot path bounded.
    let runs;
    for (const [id, mtimeMinsAgo] of [['old', 100], ['mid', 50], ['new', 5]]) {
      runs = writeLedger(root, id, header({ runId: id, heartbeat: iso(60) }));
      const t = new Date(NOW.getTime() - mtimeMinsAgo * 60_000);
      utimesSync(join(runs, id, 'ledger.md'), t, t);
    }
    const found = collectResumable(runs, { root, now: NOW, maxScan: 2 });
    assert.equal(found.length, 2, 'only the newest maxScan ledgers are read');
    assert.deepEqual(found.map((f) => f.runId).sort(), ['mid', 'new']); // 'old' dropped
  });
});
