...
// Main worktree FIRST. `.afk/` has one canonical location, so a linked
// worktree's own stray copy must not outrank it — a per-run `implementer` value
// left behind in some worktree would otherwise decide which gate runs. The
// current directory and the top level are fallbacks for when the main worktree
// cannot be determined at all.
export function findAfkConfig() {
  const main = mainWorktree();
  const top = git(['rev-parse', '--show-toplevel']).trim();
  const candidates = [
    main && join(main, '.afk', 'config.md'),
...
export function readConfigImplementer(configPath) {
  return readConfigValue(configPath, 'implementer');
}
